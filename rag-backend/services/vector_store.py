...
class VectorStore:
    ...
    def find_similar(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Tuple[Dict[str, Any], float]]:
        ...
        if self.qdrant_client is not None:
            ...
            try:
                ...
                # If results are empty, try to get some results anyway by using scroll
                if not results:
                    logger.info(f"Qdrant search returned no results, trying scroll fallback...")
                    try:
                        # Get some points from the collection as a fallback
                        scroll_result = self.qdrant_client.scroll(
                            collection_name=self.collection_name,
                            limit=min(top_k * 2, 100),  # Get more than needed for fallback
                            with_payload=True
                        )

                        # Calculate similarity manually for these fallback results
                        from services.embedding_generator import get_embedding_generator
                        results = []
                        query_text = "query"  # This is a fallback, so we'll use a generic approach
                        ...
                        for i, (point_id, payload) in enumerate(zip(scroll_result[0], scroll_result[1])):
                            ...
                            if content_text:
                                # Generate embedding for this content and calculate similarity
                                try:
                                    content_embedding = get_embedding_generator().generate_embedding(content_text)
                                    # Calculate cosine similarity
                                    dot_product = np.dot(query_embedding, content_embedding)
                                    norm_product = query_embedding_norm * np.linalg.norm(content_embedding)
                                    if norm_product != 0:
                                        similarity = dot_product / norm_product
                                    else:
                                        similarity = 0
                                except:
                                    # If embedding generation fails, use a default low similarity
                                    similarity = 0.01
                            else:
                                similarity = 0.01
                ...
                # As a last resort, try the scroll fallback
                try:
                    logger.info("Trying scroll fallback due to search error...")
                    scroll_result = self.qdrant_client.scroll(
                        collection_name=self.collection_name,
                        limit=top_k,
                        with_payload=True
                    ...
                    for i, (point_id, payload) in enumerate(zip(scroll_result[0], scroll_result[1])):
                        if i >= top_k:
                            break
                        # Create a mock result object with low similarity
                        mock_result = type('MockResult', (), {
                            'id': point_id,
                            'payload': payload,
                            'score': 0.05  # very low score due to error condition
                        })()
                        results.append(mock_result)
                    ...
                    logger.error(f"Scroll fallback also failed: {fallback_error}")
                    return []
        else:
    ...
    def find_similar_texts(self, query: str, top_k: int = 5) -> List[Tuple[Dict[str, Any], float]]:
        ...
        try:
            ...
            results = self.find_similar(query_embedding, top_k)
            # Ensure we return an empty list if results is None
            return results if results is not None else []
        except Exception as e:
            logger.error(f"Error in find_similar_texts: {e}")
            return []
