...
class RetrievalService:
    ...
    def _retrieve_from_full_store(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        # If no results found but we have content in the store, try with lower similarity threshold
        # This helps when using mock embeddings which may not have strong semantic similarity
        if not formatted_results:
            logger.info("No results found with default threshold, trying with lower similarity threshold...")
            # Try to get at least some results by reducing requirements
            fallback_results = get_vector_store().find_similar_texts(query, top_k * 2)
            for content_item, similarity in fallback_results:
                formatted_result = {
                    'id': content_item['id'],
                    'content': content_item.get('content', ''),
                    'metadata': content_item['metadata'],
                ...
                if formatted_result not in formatted_results:  # Avoid duplicates
                    formatted_results.append(formatted_result)
                if len(formatted_results) >= top_k:  # Limit to original top_k
                    break
