...
def generate_response_from_content(query: str, content_list: List[Dict[str, Any]], conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
    ...
    # Use the configured LLM (Gemini) to generate the response
    try:
        ...
        # Use the model specified in settings, but remove the "gemini/" prefix if present
        model_name = settings.llm_model.replace('gemini/', '')
        model = genai.GenerativeModel(model_name)

        response = model.generate_content(prompt)
        return response.text if response.text else "I found relevant information but couldn't generate a complete response based on the book content."
    except ImportError:
        logger.warning("Google Generative AI library not installed. Install with: pip install google-generativeai")
        # Fallback to simple response if library not available
        if context_text.strip():
            return f"Based on the book content:\n\n{context_text[:1000]}"
        else:
            return "I couldn't find the relevant information in the book to answer your question."
    except Exception as e:
        logger.error(f"Error generating response with LLM: {str(e)}")
        # Fallback to simple response if LLM call fails
        if context_text.strip():
            return f"Based on the book content:\n\n{context_text[:1000]}"
        else:
            return "I couldn't find the relevant information in the book to answer your question."